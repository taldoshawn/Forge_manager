...
object FileTypeClassifier {
    private val androidPackages = setOf("apk", "apks", "xapk", "apkm", "xapb", "aab")
    ...
    private val subtitles = setOf("srt", "ass", "ssa", "vtt", "sub")
    ...
    private val executables = setOf("so", "dll", "elf", "bin", "exe", "msi", "appimage")
    ...
    fun shortLabel(kind: FileKind, name: String): String = when (kind) {
        ...
        FileKind.APK -> when (extensionOf(name)) { "aab" -> "Android App Bundle"; else -> "Pacote Android" }
        ...
        FileKind.EXECUTABLE -> "Binário / executável"
