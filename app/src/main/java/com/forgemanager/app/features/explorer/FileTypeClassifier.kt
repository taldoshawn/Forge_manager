...
object FileTypeClassifier {
    ...
    private val code = setOf(
        ...
        "c", "cc", "cpp", "cxx", "h", "hpp", "cs", "swift", "dart", "lua", "sql", "r", "scala", "groovy", "gradle"
    )
    ...
    fun classify(name: String, isDirectory: Boolean): FileKind {
        ...
        return when {
            ...
            ext in scripts -> FileKind.SCRIPT
            ext in web -> FileKind.WEB
            ext in markdown -> FileKind.MARKDOWN
            ...
            ext in documents -> FileKind.DOCUMENT
            ext in configs -> FileKind.CONFIG
            ext in certificates -> FileKind.CERTIFICATE
            ext in executables -> FileKind.EXECUTABLE
            ext in text || name.equals("README", true) || name.equals("LICENSE", true) || name.equals("NOTICE", true) || name.equals("CHANGELOG", true) -> FileKind.TEXT
